...
int32_t hal_gpio_init(gpio_dev_t *gpio)
{
	...
	uint16_t gpio_pin,st_pin;
	GPIO_TypeDef *st_gpio;
	
    gpio_pin = get_pin(gpio->port);
	st_gpio = get_st_gpio(gpio_pin);
	st_pin = get_st_pin(gpio_pin);
    ...
    return GPIO_Init(st_gpio, &GPIO_InitStruct);
}
int32_t hal_gpio_output_high(gpio_dev_t *gpio)
{
    ...
    gpio_pin = get_pin(gpio->port);
	gpio = get_st_gpio(gpio_pin);
	pin = get_st_pin(gpio_pin);
	
    if (gpio == NULL)
    {
        return -1;
    }
    return GPIO_WriteBit(gpio,pin);	
}
int32_t hal_gpio_output_low(gpio_dev_t *gpio)
{
    ...
    return GPIO_ResetBits(gpio,pin);
}
int32_t hal_gpio_output_toggle(gpio_dev_t *gpio)
{
    ...
    return GPIO_WriteBit(gpio,pin,!GPIO_ReadOutputDataBit(gpio,pin));
}
int32_t hal_gpio_input_get(gpio_dev_t *gpio, uint32_t *value)
{
    ...
    *value = GPIO_ReadInputDataBit(gpio,pin);
